...
def get_qemu_best_cpu_info(params):
    """
    Get the best cpu model and features available for qemu
    """
    cpu_info = {}
    cpu_model = get_qemu_best_cpu_model(params)
    # Expand if needed
    cpu_flags = ""
...
def parse_qemu_cpu_flags(cpu_flags_str):
    """
    Convert cpu flags string to a dict
    """
    flags_dict = {}
    if cpu_flags_str:
        ...
        for item in flags_list:
            if item.startswith("+"):
                flags_dict[item[1:]] = "on"
            elif item.startswith("-"):
                flags_dict[item[1:]] = "off"
            elif item.find("=") != -1:
                item = item.split("=")
                flags_dict[item[0]] = item[1]
            else:
                flags_dict[item] = None
    return flags_dict


def get_cpu_info_from_virsh_qemu_cli(name):
    """
    Get cpu info from qemu cmdline generated by virsh
    """
...
def recombine_qemu_cpu_flags(base, suggestion):
    ...
    """
    flags_dict = parse_qemu_cpu_flags(base)
    tmp_dict = parse_qemu_cpu_flags(suggestion)
    flags_dict.update(tmp_dict)
    flag_info = ""
    for key, value in flags_dict.items():
        if value:
            flag_info += ",%s=%s" % (key, value)
        else:
            flag_info += ",%s" % key
    return flag_info
